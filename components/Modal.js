...
function Modal({ title, onClose = null, fullHeight = false, children, className = '', open = false }) {
    ...
    return (
        <div ref={modalRef} style={{ height: fullHeight ? '100%' : 'auto', zIndex: 999999 }} className={`pointer-events-auto bg-slate-100 dark:bg-slate-800 z-50 border shadow-lg w-full max-w-screen-sm backdrop-blur ${fullHeight ? 'rounded-tl-xl rounded-tr-xl' : 'rounded'} fixed left-0 right-0 mx-auto top-full transition-all duration-300 bg-white outline-none ${className}`}>
            <div className="w-full h-8 mr-4 flex flex-row items-center my-2 p-2">
                <div onClick={closeModal} className="text-gray-800 dark:text-white">
